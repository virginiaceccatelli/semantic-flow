...
def main(
    ...
    output: Optional[Path] = typer.Option(None, help="Default results/jlens/{model}/validate"),
    ...
    from src.experiments.jlens_validate import run_validation
    ...
    output = output or Path("results/jlens") / model / "validate"
    if lens_out:
    ...
    if tables and not df.empty:
        ...
        shutil.copy(output / "jlens_validation.csv",
                    tables_dir / f"jlens_validation_{model}.csv")
        shutil.copy(output / "jlens_validation_checks.csv",
                    tables_dir / f"jlens_validation_checks_{model}.csv")

    failed = [c.name for c in checks if c.required and not c.passed]
    write_manifest("60_jlens_validate", {
