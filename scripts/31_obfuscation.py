#!/usr/bin/env python3
"""Stage 31 (CPU): E9 — frozen probes evaluated on obfuscation-ladder variants.

Prerequisites:
    stage 10 run on data/synthetic/obfuscation.jsonl  (GPU)
...
@app.command()
def main(
    activations: Path = typer.Option(..., help="Stage-10 store over obfuscation.jsonl"),
    probes: Path = typer.Option(..., help="Stage-20 output dir with frozen checkpoints"),
    output: Optional[Path] = typer.Option(None, help="Default results/obfuscation/{model}"),
    ...
    from src.experiments.obfuscation_robustness import run_obfuscation_robustness
    from src.utils import write_manifest

    t0 = time.time()
    store = ActivationStore(activations)
    model = store.meta["model"]
    output = output or Path("results/obfuscation") / model
    task_list = [t.strip() for t in tasks.split(",")] if tasks else None

    df = run_obfuscation_robustness(store, probes, output, tasks=task_list, seed=seed)

    if tables:
        tables_dir = Path("results/tables")
        tables_dir.mkdir(parents=True, exist_ok=True)
        table_path = tables_dir / f"obfuscation_robustness_{model}.csv"
        shutil.copy(output / "obfuscation_robustness.csv", table_path)
        console.print(f"Table → {table_path}")

    write_manifest("31_obfuscation", {
        "activations": str(activations), "probes": str(probes),
        "output": str(output), "seed": seed,
    }, t0, extra={"n_rows": len(df)})
    console.print("[green]Stage 31 done.[/green]")
