...
control programs added by `--n-control`).
    ...
    python scripts/62_clens_controldep.py --model deepseek-coder-1.3b
"""
...
def main(
    ...
    output: Optional[Path] = typer.Option(None, help="Default results/clens/{model}/controldep"),
    ...
    from src.experiments.clens_controldep import run_clens_controldep
    ...
    output = output or Path("results/clens") / model / "controldep"
    if lens_out:
        Path(lens_out).mkdir(parents=True, exist_ok=True)

    df = run_clens_controldep(
    ...
    if tables and not df.empty:
        ...
        for name in ("clens_controldep.csv", "clens_controldep_summary.csv"):
            shutil.copy(output / name, tables_dir / f"{Path(name).stem}_{model}.csv")

    write_manifest("62_clens_controldep", {
