...
finite (Phase 0.3) — are asserted by `scripts/60_clens_validate.py`, which
...
from src.experiments.clens_controldep import build_guard_cases, summarize as cd_summarize
from src.experiments.clens_taint import _calibrate_margin, _first_wrong
from src.experiments.clens_taint import summarize as taint_summarize
from src.experiments.clens_validate import single_token_candidates
from src.models.cotangent_lens import CotangentLens, LensSample, lens_filename, random_lens
from tests.fake_tokenizer import FakeCharTokenizer


def _lens(vectors, kind="clens", layer=3) -> CotangentLens:
    v = np.asarray(vectors, dtype=np.float32)
    return CotangentLens(vectors=v, token_ids=list(range(v.shape[0])),
...
def test_vector_count_must_match_tokens():
    with pytest.raises(ValueError):
        CotangentLens(vectors=np.zeros((2, 4)), token_ids=[1, 2, 3],
...
def test_lens_filename_handles_embedding_layer():
    assert lens_filename("clens", -1) == "clens_layer_emb.pkl"
    assert lens_filename("clens", 7) == "clens_layer_07.pkl"
...
def test_taint_summary_matches_stage40_schema():
    ...
    df = pd.DataFrame([
        ...
        {"layer": 7, "model_ever_wrong": True, "t_failure": 5, "failure_index": 4,
         "t_latent_clens": 3, "error_rate_clens": 0.2,
         "lead_clens": 2, "latent_first_clens": True,
        ...
        {"layer": 7, "model_ever_wrong": True, "t_failure": 4, "failure_index": 3,
         "t_latent_clens": None, "error_rate_clens": 0.2,
         "lead_clens": None, "latent_first_clens": False,
         "error_rate_position": 0.3},
        # model right -> outside the denominator entirely
        {"layer": 7, "model_ever_wrong": False, "t_failure": None,
         "failure_index": None, "t_latent_clens": 2, "error_rate_clens": 0.2,
         "lead_clens": None, "latent_first_clens": False,
         "error_rate_position": 0.3},
    ])
...
def test_taint_summary_flags_constant_readouts():
    ...
    df = pd.DataFrame([{
        "layer": 7, "model_ever_wrong": True, "t_failure": 4, "failure_index": 3,
        "t_latent_clens": 2, "error_rate_clens": 0.2,
        "lead_clens": 2, "latent_first_clens": True, "error_rate_position": 0.3,
    }])
    prefix = pd.DataFrame([
        {"layer": 7, "clens_says": 1, "truth": 1},
        {"layer": 7, "clens_says": 1, "truth": 0},      # never predicts 0
    ])
...
def test_stage61_records_position_floor_and_failure_index():
    ...
    import inspect
    from src.experiments import clens_taint
    src = inspect.getsource(clens_taint.run_clens_taint)
    for needed in ('"failure_index"', 'preds["position"]', "behavioural_sanity",
                   "clens_taint_prefixes.csv"):
...
def test_controldep_summary_tolerates_pre_control_runs():
    ...
    df = pd.DataFrame([
        {"layer": 3, "lens": "clens", "stratum": "indent_matched",
...
def test_temporal_split_reported_as_its_own_stratum():
    ...
    df = pd.DataFrame([
        {"layer": 7, "lens": "clens", "comparison": "control_dep",
         "stratum": "indent_matched", "margin": 1.0, "correct": True,
         "negative_after": True},
        {"layer": 7, "lens": "clens", "comparison": "control_dep",
         "stratum": "indent_matched", "margin": -1.0, "correct": False,
...
def test_stage90_renders_stage61_summary_schema(tmp_path):
    ...
    mod = _stage90(tmp_path)
    csv = tmp_path / "clens_taint_summary_M.csv"
    pd.DataFrame([
        {"layer": L, "readout": r, "n_model_wrong": 3,
         "per_prefix_error_rate": 0.1, "early_warning_rate": 0.5,
         "analytic_null": 0.4, "early_warning_excess": 0.1,
         "constant_readout": r == "dead", "beats_position_floor": True,
         "readout_never_wrong": 1, "mean_lead": 1.0}
        for L in (0, 7) for r in ("clens", "logit", "random", "probe", "dead")
    ]).to_csv(csv, index=False)
    mod._clens_taint_assets(csv)
    assert (tmp_path / "clens_taint_excess_M.png").exists()
    assert (tmp_path / "clens_taint_earlywarning_M.png").exists()
...
def test_stage90_registers_every_stage61_output():
    ...
    spec.loader.exec_module(mod)
    src = inspect.getsource(mod.main)
    for prefix in ("clens_taint_summary_", "clens_taint_sanity",
                   "clens_taint_prefixes", "clens_taint_"):
