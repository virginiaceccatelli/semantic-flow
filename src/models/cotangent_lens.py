...
Because the shared `1/rms(J_l h)` factor is dropped, `CotangentLens.scores(h)` is
...
from the weights, and `scripts/60_clens_validate.py` asserts the two agree
...
class CotangentLens:
    ...
    `kind` records how it was built ("clens" | "logit" | "random" |
    ...
    @classmethod
    def load(cls, path: str | Path) -> "CotangentLens":
        with open(path, "rb") as f:
            state = pickle.load(f)
        return cls(**state)


def load_frozen_lenses(lens_dir: str | Path, kind: str = "clens") -> dict[int, CotangentLens]:
    ...
    lenses: dict[int, CotangentLens] = {}
    for path in sorted(lens_dir.glob(f"{kind}_layer_*.pkl")):
        lens = CotangentLens.load(path)
...
def compute_lens_vectors(
    ...
    forward+backward pass and the result is an **R-lens** (`kind="clrp"`) —
...
def random_lens(reference: CotangentLens, seed: int = 42) -> CotangentLens:
...
def gram_matched_random_lens(reference: CotangentLens, seed: int = 42) -> CotangentLens:
