...
def next_token_metrics(lens: CotangentLens, evals: Sequence[tuple[np.ndarray, int]]) -> dict:
...
def disposition_metrics(
    lens: CotangentLens, evals: Sequence[tuple[np.ndarray, int, int]],
) -> dict:
...
def run_validation(
    ...
    for layer in layers:
        ...
        for kind, lens in (("clens", nt_lens), ("logit", nt_logit), ("random", nt_random)):
        ...
        if tt_build and taint_answers:
            ...
            for kind, lens in (("clens", tt_lens), ("logit", tt_logit), ("random", tt_random)):
        ...
        if lens_dir is not None:
            ...
            nt_lens.save(lens_path / lens_filename("clens_nexttoken", layer))

    df = pd.DataFrame(rows)
    df.to_csv(output_dir / "clens_validation.csv", index=False)

    checks += _phase1_gates(df, last_layer, last_layer_cosines)
    checks_df = pd.DataFrame([c.as_row() for c in checks])
    checks_df.to_csv(output_dir / "clens_validation_checks.csv", index=False)
    return df, checks
...
def _best_early_gain(
    ...
    jl = sub[sub["lens"] == "clens"].set_index("layer")[col]
...
def _phase1_gates(df: pd.DataFrame, last_layer: int, last_layer_cosines: list[float]) -> list[Check]:
    ...
    def _paired(check: str, col: str, against: str) -> tuple:
        ...
        jl = sub[sub["lens"] == "clens"].dropna(subset=[col])
        if jl.empty:
    ...
    if np.isfinite(j2) and np.isfinite(r2):
        checks.append(Check(
            ...
            detail=f"layer {layer2} top-1: clens {j2:.3f} vs random {r2:.3f} "
                   f"(logit {l2:.3f}), n={n2}"
    ...
    if np.isfinite(j3) and np.isfinite(r3):
        checks.append(Check(
            ...
            detail=f"layer {layer3} model-agreement: clens {j3:.3f} vs random "
                   f"{r3:.3f}, n={n3}"
                   + ("" if n3 >= 10 else "  [UNDERPOWERED: need n>=10]"),
        ))
        sub = df[(df["check"] == "V3_taint_disposition") & (df["lens"] == "clens")
                 & (df["layer"] == last_layer)]
        if not sub.empty:
