...
`results/tables/jlens_validation_*.csv`. The LRP backward holds 0.945-1.005 at
...
from src.experiments.jlens_validate import Check, next_token_samples, single_token_candidates
...
from src.models.lrp import lrp_rules  # noqa: F401  (re-exported for stage 110)
...
def run_gate_r(
    ...
    checks.append(r0)
    r0_rows.to_csv(output_dir / "rlens_r0_forward.csv", index=False)
    ...
    df.to_csv(output_dir / "rlens_r2_conservation.csv", index=False)
