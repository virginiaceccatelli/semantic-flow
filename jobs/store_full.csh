...
set PAIRS = "data/synthetic/store_pairs_${MODEL}.jsonl"
...
$RUN scripts/81_store_verify.py --model "$MODEL" --pairs "$PAIRS" --strict || exit 1

echo "=== stage 82: can the model solve them? (G1) — GPU ==="
$RUN scripts/82_store_behaviour.py --model "$MODEL" --pairs "$PAIRS" \
    --dtype "$DTYPE" --strict || exit 1

echo "=== stage 83: cache anchor states — GPU ==="
$RUN scripts/83_store_extract.py --model "$MODEL" --pairs "$PAIRS" \
    --layers "$LAYERS" --dtype "$DTYPE" || exit 1

echo "=== stage 84: decodability (G2) — CPU ==="
$RUN scripts/84_store_decode.py --model "$MODEL" --pairs "$PAIRS" --strict || exit 1

echo "=== stage 85: natural transitions (G3) — CPU ==="
$RUN scripts/85_store_transition.py --model "$MODEL" --pairs "$PAIRS" --strict || exit 1

echo "=== stage 86: whole-state interchange ceiling (G4) — GPU ==="
$RUN scripts/86_store_ceiling.py --model "$MODEL" --pairs "$PAIRS" \
    --layers "$LAYERS" --dtype "$DTYPE" --strict || exit 1

echo "=== stage 87: DAS low-rank interchange + six controls (G5) — GPU ==="
$RUN scripts/87_store_interchange.py --model "$MODEL" --pairs "$PAIRS" \
...
echo "choosing a semantic extension in docs/design/E13_DIRECTIONS.md."
